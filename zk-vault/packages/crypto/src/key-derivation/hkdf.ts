...
export class HKDF {
  ...
  static async derive(
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.crypto?.subtle) {
        ...
        derivedKey = await this.deriveWithStableLib(
          inputKeyMaterial,
          salt,
          info,
          outputLength
        ...
        derivedKey = await this.deriveManual(
          inputKeyMaterial,
          salt,
          info,
          outputLength,
  ...
  private static async deriveWithWebCrypto(
    ...
    try {
      ...
      throw new Error(`WebCrypto HKDF failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private static async deriveWithNodeCrypto(
    ...
    try {
      ...
      throw new Error(`Node.js HKDF failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private static async deriveWithStableLib(
    ...
    try {
      ...
      throw new Error(`StableLib HKDF failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private static async deriveManual(
    ...
    try {
      ...
      throw new Error(`Manual HKDF failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
