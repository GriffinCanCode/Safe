<template>
  <div ref="itemElement" class="password-item card-lift" :class="itemClasses">
    <!-- Main Content -->
    <div class="item-content" @click="handleItemClick">
      <!-- Icon and Info -->
      <div class="item-info">
        <div ref="iconElement" class="item-icon" :class="iconClasses">
          <img
            v-if="item.iconUrl"
            :src="item.iconUrl"
            :alt="`${item.name} icon`"
        ...
        <div class="item-details">
          ...
          <div class="item-meta">
            ...
            <span class="meta-updated"> Updated {{ formatRelativeTime(item.updatedAt) }} </span>
          </div>
        </div>
      </div>
    ...
    <div class="item-actions">
      ...
      <div class="more-actions-menu" v-if="showActions">
        ...
        <div
          ...
          @click.stop
        >
          <button class="dropdown-item btn-hover" @click="editItem">
          ...
          <button class="dropdown-item btn-hover" @click="duplicateItem">
            <svg class="dropdown-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <button class="dropdown-item btn-hover" @click="shareItem">
            <svg class="dropdown-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <button class="dropdown-item danger btn-hover" @click="deleteItem">
            <svg class="dropdown-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
    ...
    <!-- Copy Success Toast -->
    <div v-if="showCopySuccess" ref="copyToastElement" class="copy-toast animate-fade-in">
...
const securityWarnings = computed((): SecurityWarning[] => {
  ...
  const daysSinceUpdate = Math.floor(
    (Date.now() - props.item.updatedAt.getTime()) / (1000 * 60 * 60 * 24)
  );
