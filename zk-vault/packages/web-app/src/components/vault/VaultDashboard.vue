  ...
  <div class="vault-dashboard">
    ...
    <div class="dashboard-header">
      ...
      <div class="header-actions">
        <BaseButton variant="primary" @click="showPasswordEditor = true" icon="plus">
          Add Password
        </BaseButton>
        <BaseButton variant="outline" @click="showGenerator = true" icon="key">
          Generate Password
        </BaseButton>
      </div>
    </div>
          ...
          <p class="security-description">
            {{
              stats.weakPasswords === 0
                ...
                : 'Some passwords need strengthening.'
            }}
          </p>
          <BaseButton
          ...
          <p class="security-description">
            {{
              stats.compromisedPasswords === 0
                ...
                : 'Some passwords have been found in data breaches.'
            }}
          </p>
          <BaseButton
          ...
          <p class="security-description">
            {{
              stats.reusedPasswords === 0
                ...
                : 'Some passwords are used multiple times.'
            }}
          </p>
          <BaseButton
    ...
    <div class="recent-items" v-if="recentItems.length > 0">
      <div class="section-header">
        ...
        <router-link to="/vault/passwords" class="view-all-link"> View All </router-link>
      </div>
      <div class="recent-items-grid">
        <PasswordItem
          v-for="item in recentItems"
          :key="item.id"
          :item="convertPasswordToVaultItem(item)"
          @edit="handleEditPassword"
          @delete="handleDeletePassword"
    ...
    <PasswordEditor
      ...
      v-bind="editingPassword ? { item: editingPassword } : {}"
...
const convertPasswordToVaultItem = (item: PasswordVaultItem) => {
  const vaultItem: any = {
    id: item.id,
    name: item.name,
...
const handleEditPassword = (vaultItem: any) => {
  // Find the original PasswordVaultItem by ID
  const originalItem = recentItems.value.find(item => item.id === vaultItem.id);
  if (originalItem) {
    editPassword(originalItem);
  }
};

const handleDeletePassword = (vaultItem: any) => {
  deletePassword(vaultItem.id);
};

const handleToggleFavorite = (vaultItem: any, favorite: boolean) => {
  // Find the original PasswordVaultItem by ID
  const originalItem = recentItems.value.find(item => item.id === vaultItem.id);
  if (originalItem) {
    toggleFavorite({ ...originalItem, favorite });
  }
};
