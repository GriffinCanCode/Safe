    ...
    <div class="item-content">
          ...
          <div class="action-menu" v-if="viewMode === 'list'">
            ...
            <div v-if="showActions" class="actions-dropdown" @click.stop>
              ...
              <button class="dropdown-action" @click="handleCopy('password')">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 14l-1 1-1 1H7v4H3v-4l4-4.257a6 6 0 017.743-7.743z" />
                </svg>
                Copy Password
              </button>
      ...
      <div class="item-details">
        ...
        <template v-else-if="item.type === 'note'">
          <div class="detail-row">
            <span class="detail-value note-content" v-html="highlightMatches(getNotePreview())"></span>
          </div>
        </template>
...
<style scoped>
.search-result-item {
  @apply bg-white border border-neutral-200 rounded-lg transition-all duration-200;
  @apply hover:border-neutral-300 hover:shadow-sm cursor-pointer;
}
...
.search-result-grid {
  @apply relative p-4 hover:shadow-md;
}
...
/* Item Icon */
.item-icon {
  @apply flex-shrink-0 w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center;
  @apply text-primary-600;
}

.search-result-grid .item-icon {
  @apply w-8 h-8;
}
...
.search-result-grid .item-content {
  @apply space-y-3;
}
...
.action-button {
  @apply p-1 text-neutral-400 hover:text-neutral-600 transition-colors rounded;
}
...
.actions-dropdown {
  @apply absolute right-0 top-full mt-1 bg-white border border-neutral-200 rounded-lg shadow-lg z-10;
  @apply min-w-40;
}

.dropdown-action {
  @apply w-full flex items-center gap-2 px-3 py-2 text-left text-sm;
  @apply hover:bg-neutral-50 transition-colors first:rounded-t-lg last:rounded-b-lg;
}
...
.search-result-grid .item-details {
  @apply space-y-2;
}
...
.search-result-grid .detail-row {
  @apply text-xs;
}

.detail-label {
  @apply text-neutral-500 font-medium min-w-0 flex-shrink-0;
}

.detail-value {
  @apply text-neutral-700 min-w-0;
}
...
/* Item Meta */
.item-meta {
  @apply flex items-center justify-between gap-2 text-xs text-neutral-500;
}

.search-result-grid .item-meta {
...
.meta-tag {
  @apply px-2 py-0.5 bg-neutral-100 text-neutral-600 rounded-full text-xs;
}
...
.search-result-grid:hover .grid-actions {
  @apply opacity-100;
}

.grid-action {
  @apply p-1.5 bg-white border border-neutral-200 rounded shadow-sm;
  @apply text-neutral-600 hover:text-neutral-900 hover:border-neutral-300;
...
@media (prefers-color-scheme: dark) {
  ...
  .dropdown-action.delete {
    @apply text-red-400 hover:bg-red-900/20;
  }
  ...
  .grid-action {
    ...
    @apply hover:text-neutral-200 hover:border-neutral-500;
  }
