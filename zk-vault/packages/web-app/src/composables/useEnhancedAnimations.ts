...
export function useEnhancedAnimations() {
  ...
  const createAnimation = (element: Ref<HTMLElement | undefined>, config: AnimationConfig) => {
    ...
    switch (direction) {
      ...
      case 'scale':
        ...
        variants.animate = {
          opacity: 1,
          scale: 1,
          transition: { duration: 0.5, ease: 'cubic-bezier(0.175, 0.885, 0.32, 1.275)' },
        };
        break;
      default:
  ...
  const createStaggeredEntrance = (elements: Ref<HTMLElement[]>) => {
  ...
  const createErrorAnimation = (element: Ref<HTMLElement | undefined>) => {
    const variants: MotionVariants = {
      error: {
        transform:
          'translateX(-10px) translateX(10px) translateX(-10px) translateX(10px) translateX(0px)',
