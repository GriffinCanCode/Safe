...
exports.optimizeVault =
  exports.getVaultAnalytics =
  exports.createAutomaticBackup =
  exports.importVault =
  exports.exportVault =
  exports.searchVaultItems =
  exports.createVaultFolder =
  exports.BackupType =
  exports.VaultItemType =
    void 0;
...
exports.createVaultFolder = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (parentId) {
      const parentDoc = await db.collection("vaultFolders").doc(parentId).get();
...
exports.searchVaultItems = functions.https.onCall(async (data, context) => {
  try {
    ...
    const userId = context.auth.uid;
    // Build base query
    let query = db.collection("vaults").doc(userId).collection("items");
    // Add filters
...
exports.optimizeVault = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (operations.includes("all") || operations.includes("cleanup_backups")) {
      results.oldBackupsRemoved = await cleanupOldBackups(userId);
    }
    // Rebuild search indexes
    if (operations.includes("all") || operations.includes("rebuild_indexes")) {
