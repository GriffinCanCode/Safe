...
exports.cleanupOldAuditLogs = exports.getSystemAuditAnalytics = exports.getUserAuditStats = exports.getUserAuditLogs = exports.logVaultAuditEvent = exports.AuditEventType = void 0;
...
exports.logVaultAuditEvent = functions.https.onCall(async (data, context) => {
    try {
        ...
        const { eventType, itemId, itemType, targetUserId, success = true, errorCode, additionalData = {}, ipAddress, userAgent, deviceInfo, } = data;
        // Validate required fields
        if (!eventType || !Object.values(AuditEventType).includes(eventType)) {
            throw new functions.https.HttpsError("invalid-argument", "Valid event type is required");
        }
        // Create audit entry
        const auditEntry = {
...
exports.getUserAuditLogs = functions.https.onCall(async (data, context) => {
    try {
        ...
        const { startDate, endDate, eventTypes = [], limit = 50, offset = 0, } = data || {};
        ...
        if (startDate) {
            query = query.where("metadata.timestamp", ">=", admin.firestore.Timestamp.fromDate(new Date(startDate)));
        }
        if (endDate) {
            query = query.where("metadata.timestamp", "<=", admin.firestore.Timestamp.fromDate(new Date(endDate)));
        }
        // Apply pagination
...
exports.getUserAuditStats = functions.https.onCall(async (data, context) => {
    try {
        ...
        snapshot.docs.forEach((doc) => {
            ...
            const day = data.metadata.timestamp?.toDate()?.toISOString().split("T")[0] || "";
            // Count by event type
        ...
        stats.mostCommonEventType =
            Object.entries(stats.eventsByType).sort(([, a], [, b]) => b - a)[0]?.[0] || "";
 ...
 */
exports.getSystemAuditAnalytics = functions.https.onCall(async (data, context) => {
    try {
        // Validate context
        const validation = (0, validation_utils_1.validateFunctionContext)(context);
        if (!validation.isValid) {
        ...
        analytics.sharingMetrics.activeShares = Math.max(0, analytics.sharingMetrics.totalShares -
            analytics.sharingMetrics.totalUnshares);
        return {
            success: true,
            period,
    ...
    try {
        ...
        if (eventType === AuditEventType.VAULT_EXPORT) {
            ...
            const recentExports = await db
                ...
                .where("metadata.timestamp", ">=", admin.firestore.Timestamp.fromDate(oneHourAgo))
                .get();
        ...
        if (eventType === AuditEventType.VAULT_ITEM_SHARED) {
            ...
            const recentShares = await db
                ...
                .where("metadata.timestamp", ">=", admin.firestore.Timestamp.fromDate(oneDayAgo))
                .get();
...
exports.cleanupOldAuditLogs = functions.pubsub
    ...
    try {
        ...
        const oldLogsSnapshot = await db
            .collection("vaultAuditLogs")
            .where("metadata.timestamp", "<", admin.firestore.Timestamp.fromDate(oneYearAgo))
