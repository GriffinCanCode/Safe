...
exports.generateChunkRecoveryPlan = exports.validateChunkEncryption = exports.optimizeChunk = exports.cleanupExpiredChunks = exports.verifyChunkIntegrity = void 0;
...
exports.verifyChunkIntegrity = functions.storage
    ...
    try {
        ...
        if (chunkSize <= 0 ||
            ...
            console.warn(`Chunk size out of expected range: ${chunkSize} bytes, expected ~${avgChunkSize} bytes`);
        ...
        await fileRef.update({
            [`chunkMetadata.${chunkIndex}`]: {
                ...
                timeCreated: admin.firestore.Timestamp.fromDate(new Date(object.timeCreated || Date.now())),
                verified: true,
            },
        });
        return null;
...
exports.cleanupExpiredChunks = functions.pubsub
    ...
    try {
        ...
        console.log(`Found ${expiredFilesSnapshot.size} expired file records to clean up`);
        ...
        const deletePromises = expiredFilesSnapshot.docs.map(async (doc) => {
            ...
            console.log(`Cleaned up expired file ${fileId} with ${fileData.chunkPaths.length} chunks`);
            return fileId;
        });
...
exports.optimizeChunk = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "optimizeChunk", 20);
        const { fileId, chunkIndex } = data;
        // Validate input
        if (!fileId || chunkIndex === undefined) {
            throw new functions.https.HttpsError("invalid-argument", "Missing fileId or chunkIndex");
        }
        // Get file record
        const fileRef = db.collection("files").doc(fileId);
        const fileDoc = await fileRef.get();
        ...
        if (!chunkPath) {
            throw new functions.https.HttpsError("invalid-argument", "Invalid chunk index");
        }
        // Check if chunk exists
        const [exists] = await bucket.file(chunkPath).exists();
        if (!exists) {
            throw new functions.https.HttpsError("not-found", "Chunk not found in storage");
        }
        ...
        await fileRef.update({
            ...
            [`chunkMetadata.${chunkIndex}.optimizedAt`]: admin.firestore.FieldValue.serverTimestamp(),
        });
        return {
            success: true,
 ...
 */
exports.validateChunkEncryption = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "validateChunkEncryption", 20);
        ...
        const validationResults = {
            hasValidHeader: encryptionHeaders.version && encryptionHeaders.algorithm,
        ...
        const isValid = validationResults.hasValidHeader &&
        ...
        await fileRef.update({
            ...
            [`chunkMetadata.${chunkIndex}.encryptionValidationResults`]: validationResults,
            [`chunkMetadata.${chunkIndex}.validatedAt`]: admin.firestore.FieldValue.serverTimestamp(),
        });
        return {
            success: true,
            isValid,
 ...
 */
exports.generateChunkRecoveryPlan = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "generateChunkRecoveryPlan", 5);
        const { fileId, corruptedChunks } = data;
        // Validate input
        if (!fileId || !Array.isArray(corruptedChunks)) {
            throw new functions.https.HttpsError("invalid-argument", "Missing fileId or corruptedChunks array");
        }
        // Get file record
        const fileRef = db.collection("files").doc(fileId);
        const fileDoc = await fileRef.get();
        ...
        if (!fileData || !fileData.chunkPaths) {
            throw new functions.https.HttpsError("invalid-argument", "File data is incomplete");
        }
        // Check which chunks actually exist in storage
        const chunkExistencePromises = fileData.chunkPaths.map(async (path, index) => {
        ...
        const recoveryPlan = await Promise.all(allProblematicChunks.map(async (chunkIndex) => {
            const chunkPath = fileData.chunkPaths[chunkIndex];
        ...
        return (0, error_handler_1.handleError)(error, "generateChunkRecoveryPlan");
    }
});
