...
exports.deleteFile = exports.listFiles = exports.shareFile = exports.generateDownloadUrls = exports.finalizeFileUpload = exports.initiateFileUpload = void 0;
...
exports.initiateFileUpload = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "initiateFileUpload", 10);
        const { fileName, fileSize, contentType, chunkSize = crypto_1.FILE_ENCRYPTION.CHUNK_SIZE, encryptionPreferences, } = data;
        // Validate input
        ...
        if (fileSize > crypto_1.FILE_ENCRYPTION.MAX_FILE_SIZE) {
            throw new functions.https.HttpsError("invalid-argument", "File size exceeds maximum allowed");
        }
        // Select optimal encryption algorithm
        const algorithmSelection = await crypto_1.AlgorithmSelector.selectOptimalAlgorithm();
        // Calculate number of chunks
        const totalChunks = Math.ceil(fileSize / chunkSize);
        if (totalChunks > 10000) {
            // Reasonable limit for chunks
            throw new functions.https.HttpsError("invalid-argument", "File would exceed maximum chunk limit");
        }
        // Create file record
        ...
        const chunkPaths = Array.from({ length: totalChunks }, (_, i) => `files/${userId}/${fileId}/chunks/${i.toString().padStart(6, "0")}`);
        // Create signed URLs for chunk uploads
        const uploadUrls = await Promise.all(chunkPaths.map(async (path) => {
...
exports.finalizeFileUpload = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "finalizeFileUpload", 10);
        const { fileId, encryptedMetadata, fileHash } = data;
        // Validate input
        if (!fileId || !encryptedMetadata || !fileHash) {
            throw new functions.https.HttpsError("invalid-argument", "Missing required finalization data");
        }
        // Get file record
        ...
        if (fileData?.userId !== context.auth.uid) {
            throw new functions.https.HttpsError("permission-denied", "You do not have permission to finalize this file");
        }
        ...
        if (!allChunksUploaded) {
            throw new functions.https.HttpsError("failed-precondition", "Not all chunks have been uploaded");
        }
        // Update file record
...
exports.generateDownloadUrls = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "generateDownloadUrls", 20);
        const { fileId } = data;
        // Validate input
        if (!fileId) {
        ...
        if (!isOwner && !isShared) {
            throw new functions.https.HttpsError("permission-denied", "You do not have access to this file");
        }
        // Check if file is complete
        if (fileData?.status !== "complete") {
            throw new functions.https.HttpsError("failed-precondition", "File is not ready for download");
        }
        // Generate download URLs for all chunks
        const downloadUrls = await Promise.all(fileData.chunkPaths.map(async (path) => {
            const [url] = await bucket.file(path).getSignedUrl({
                version: "v4",
                action: "read",
                expires: Date.now() + 60 * 60 * 1000,
...
exports.shareFile = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "shareFile", 10);
        const { fileId, recipientEmail, encryptedFileKey, permissions = "read", } = data;
        // Validate input
        ...
        // Security check: ensure user owns this file
        if (fileData?.userId !== context.auth.uid) {
            throw new functions.https.HttpsError("permission-denied", "You do not own this file");
        }
        // Find recipient user
        ...
        if (recipientId === context.auth.uid) {
            throw new functions.https.HttpsError("invalid-argument", "Cannot share file with yourself");
        }
        ...
        if (!existingShare.empty) {
            throw new functions.https.HttpsError("already-exists", "File is already shared with this user");
        }
        // Create share record
...
exports.listFiles = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "listFiles", 30);
        const { includeShared = true, status = "complete", limit = 50, offset = 0, } = data || {};
        const userId = context.auth.uid;
        const files = [];
        // Get owned files
...
exports.deleteFile = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "deleteFile", 10);
        const { fileId } = data;
        // Validate input
        if (!fileId) {
            throw new functions.https.HttpsError("invalid-argument", "File ID is required");
