...
exports.getBlockedEntities = exports.unblockUser = exports.blockUser = exports.getRateLimitStatus = exports.updateRateLimitConfig = void 0;
 ...
 */
exports.updateRateLimitConfig = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Validate input
        if (!endpoint || !config) {
            throw new functions.https.HttpsError("invalid-argument", "Missing endpoint or configuration");
        }
        // Validate configuration
        if (typeof config.windowSizeMs !== "number" ||
            typeof config.maxRequests !== "number" ||
            typeof config.blockDurationMs !== "number") {
            throw new functions.https.HttpsError("invalid-argument", "Invalid rate limit configuration");
        }
        ...
        return (0, error_handler_1.handleError)(error, "updateRateLimitConfig");
    }
});
/**
...
exports.getRateLimitStatus = functions.https.onCall(async (data, context) => {
    try {
        ...
        const config = (configs && configs[endpoint]) ||
        ...
        if (userRateLimit) {
            ...
            const windowEndTime = new Date(windowStartTime.getTime() + config.windowSizeMs);
            const now = new Date();
            msUntilReset = Math.max(0, windowEndTime.getTime() - now.getTime());
            remainingRequests = Math.max(0, config.maxRequests - userRateLimit.requestCount);
            // Check if user is blocked
            if (userRateLimit.blocked) {
                const blockEndTime = userRateLimit.blockedAt.toDate().getTime() + config.blockDurationMs;
        ...
        return (0, error_handler_1.handleError)(error, "getRateLimitStatus");
    }
});
/**
...
exports.blockUser = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Validate input
        if (!userId && !ip) {
            throw new functions.https.HttpsError("invalid-argument", "Must provide userId or IP");
        }
        if (durationMs < 60000 || durationMs > 30 * 24 * 60 * 60 * 1000) {
            throw new functions.https.HttpsError("invalid-argument", "Block duration must be between 1 minute and 30 days");
        }
        ...
        if (userId) {
            ...
            batch.update(userRef, {
                ...
                blockedBy: context.auth.uid,
                blockExpiresAt: admin.firestore.Timestamp.fromDate(new Date(Date.now() + durationMs)),
            });
            // Log the action
        ...
        return (0, error_handler_1.handleError)(error, "blockUser");
    }
});
/**
...
exports.unblockUser = functions.https.onCall(async (data, context) => {
    try {
        ...
        return (0, error_handler_1.handleError)(error, "unblockUser");
    }
});
/**
 ...
 */
exports.getBlockedEntities = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated and an admin
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        const blockedIps = blockedIpsSnapshot.docs.map((doc) => {
            ...
            return {
                ...
                blockExpiresAt: data.blockedAt && data.blockDurationMs ?
                    new Date(data.blockedAt.toDate().getTime() + data.blockDurationMs) :
                    null,
            };
        });
        return {
        ...
        return (0, error_handler_1.handleError)(error, "getBlockedEntities");
    }
});
//# sourceMappingURL=rate-limiting.functions.js.map