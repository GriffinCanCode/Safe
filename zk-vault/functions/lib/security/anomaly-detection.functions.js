...
exports.detectKeyUsageAnomalies = exports.detectFileAccessAnomalies = exports.detectLoginAnomalies = void 0;
 ...
 */
exports.detectLoginAnomalies = functions.firestore
    ...
    try {
        ...
        const previousSessions = previousSessionsSnapshot.docs.map((doc) => doc.data());
        // Check for anomalies
        const anomalies = [];
        // 1. Unusual location
        if (sessionData.geoData && sessionData.geoData.country) {
            const previousCountries = new Set(previousSessions
        ...
        if (sessionData.timestamp) {
            ...
            const previousLoginHours = previousSessions
                ...
                .map((s) => s.timestamp.toDate().getHours());
            const hourFrequency = previousLoginHours.reduce((acc, hour) => {
            ...
            if (!hourFrequency[loginHour] || hourFrequency[loginHour] <= 1) {
                anomalies.push({
                    ...
                    details: {
                        loginHour,
                        usualHours: Object.keys(hourFrequency).filter((h) => hourFrequency[parseInt(h)] > 1),
                    },
                });
            }
        }
        ...
        if (sessionData.deviceInfo && sessionData.deviceInfo.userAgent) {
            const previousDevices = new Set(previousSessions
        ...
        if (sessionData.geoData &&
            sessionData.geoData.latitude &&
            sessionData.geoData.longitude &&
            sessionData.timestamp) {
            // Find most recent session with geo data
            const lastSessionWithGeo = previousSessions.find((s) => s.geoData &&
                s.geoData.latitude &&
                s.geoData.longitude &&
                s.timestamp);
            if (lastSessionWithGeo) {
                // Calculate distance and time difference
                const distance = calculateDistance(sessionData.geoData.latitude, sessionData.geoData.longitude, lastSessionWithGeo.geoData.latitude, lastSessionWithGeo.geoData.longitude);
                const timeDiff = sessionData.timestamp.toDate() -
 ...
 */
exports.detectFileAccessAnomalies = functions.firestore
    ...
    try {
        ...
        const previousAccessSnapshot = await db
            ...
            .limit(50)
            .get();
        const previousAccess = previousAccessSnapshot.docs.map((doc) => doc.data());
        // Check for anomalies
        const anomalies = [];
        ...
        if (accessCount > avgDailyAccess * 3 && accessCount > 10) {
            anomalies.push({
                ...
                details: {
                    ...
                    percentIncrease: Math.round((accessCount / avgDailyAccess - 1) * 100),
                },
            });
        }
        ...
        if (accessData.fileType) {
            ...
            if (!fileTypeFrequency[accessData.fileType] ||
                ...
                anomalies.push({
                    ...
                    details: {
                        ...
                        commonTypes: Object.keys(fileTypeFrequency).filter((t) => fileTypeFrequency[t] > 2),
                    },
                });
            }
        }
        ...
        if (accessData.sensitivity === "high") {
            ...
            if (accessData.geoData && accessData.geoData.country) {
                const previousLocationsForSensitiveFiles = new Set(previousAccess
                    .filter((a) => a.sensitivity === "high" && a.geoData && a.geoData.country)
                    .map((a) => a.geoData.country));
                if (!previousLocationsForSensitiveFiles.has(accessData.geoData.country)) {
                    anomalies.push({
                        ...
                        details: {
                            ...
                            previousLocations: Array.from(previousLocationsForSensitiveFiles),
                        },
                    });
                }
            }
 ...
 */
exports.detectKeyUsageAnomalies = functions.firestore
    ...
    try {
        ...
        if (usageCount > 20 && usageCount > previousUsageSnapshot.size * 2) {
            anomalies.push({
                ...
                details: {
                    ...
                    percentIncrease: Math.round((usageCount / (previousUsageSnapshot.size || 1) - 1) * 100),
                },
            });
        }
        // 2. Key usage from new device
        if (usageData.deviceInfo && usageData.deviceInfo.deviceId) {
            const previousDevices = new Set(previousUsageSnapshot.docs
                .filter((doc) => doc.data().deviceInfo && doc.data().deviceInfo.deviceId)
        ...
        if (usageData.operation) {
            ...
            if (!operationFrequency[usageData.operation] ||
                ...
                anomalies.push({
                    ...
                    details: {
                        ...
                        commonOperations: Object.keys(operationFrequency).filter((op) => operationFrequency[op] > 1),
                    },
                });
            }
        }
...
//# sourceMappingURL=anomaly-detection.functions.js.map