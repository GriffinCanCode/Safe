...
exports.monitorEncryptionOperations = exports.processBreachCheckTask = exports.scheduleCredentialBreachChecks = exports.checkLeakedCredentials = exports.monitorFailedLogins = void 0;
...
exports.monitorFailedLogins = functions.firestore
    ...
    try {
        ...
        const windowStart = admin.firestore.Timestamp.fromDate(new Date(Date.now() - BREACH_CONFIG.FAILED_LOGIN_WINDOW * 60 * 1000));
        ...
        if (userFailedCount >= BREACH_CONFIG.MAX_FAILED_LOGINS ||
            ...
            await db.collection("securityAlerts").add({
                ...
                severity: userFailedCount > BREACH_CONFIG.MAX_FAILED_LOGINS * 2 ?
                    "high" :
                    "medium",
                status: "new",
                details: {
 ...
 */
exports.checkLeakedCredentials = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "checkLeakedCredentials", 5);
        const { emailHash, passwordHash } = data;
        // Validate input
        if (!emailHash) {
            throw new functions.https.HttpsError("invalid-argument", "Missing emailHash");
        }
        ...
        if (passwordHash) {
            ...
            isPasswordBreached = passwordBreachResults.some((hash) => hash === passwordHash);
        }
...
exports.scheduleCredentialBreachChecks = functions.pubsub
    ...
    try {
        ...
        const checkThreshold = admin.firestore.Timestamp.fromDate(new Date(Date.now() -
        ...
        console.log(`Scheduling breach checks for ${usersToCheckSnapshot.size} users`);
...
exports.monitorEncryptionOperations = functions.firestore
    ...
    try {
        ...
        const windowStart = admin.firestore.Timestamp.fromDate(new Date(Date.now() - 60 * 60 * 1000));
        ...
        const userData = userDoc.data();
        const avgHourlyOperations = userData?.analytics?.avgHourlyEncryptionOps || 5; // Default threshold
        ...
        if (operationCount > avgHourlyOperations * 3 && operationCount > 20) {
            await db.collection("securityAlerts").add({
                ...
                details: {
                    ...
                    avgHourlyOperations,
                    percentIncrease: Math.round((operationCount / avgHourlyOperations - 1) * 100),
                },
            });
            ...
            await db
                ...
                .doc(userId)
                .update({
                "analytics.lastHighVolumeEncryptionAt": admin.firestore.FieldValue.serverTimestamp(),
                "analytics.highVolumeEncryptionCount": admin.firestore.FieldValue.increment(1),
                "securityAlerts": admin.firestore.FieldValue.increment(1),
            });
        }
        return null;
