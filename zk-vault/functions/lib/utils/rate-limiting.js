...
exports.checkPersistentRateLimit = exports.rateLimit = exports.checkRateLimit = void 0;
            ...
            if (now - windowStartTime < windowMs) {
                ...
                if (data.blocked) {
                    ...
                    if (now - blockTime < blockDuration) {
                        throw new functions.https.HttpsError("resource-exhausted", `Rate limit exceeded. Try again in ${Math.ceil((blockDuration - (now - blockTime)) / 1000)} seconds`);
                    }
                    else {
                ...
                if (data && data.requestCount >= actualMaxRequests) {
                    ...
                    throw new functions.https.HttpsError("resource-exhausted", "Rate limit exceeded. Account temporarily blocked");
                }
 ...
 */
async function checkPersistentRateLimit(userId, functionName, maxRequests, windowMs) {
    try {
        const now = Date.now();
