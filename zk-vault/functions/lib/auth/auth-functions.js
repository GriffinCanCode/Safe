...
exports.changePassword = exports.registerUser = exports.verifySRPAuth = exports.initSRPAuth = void 0;
 ...
 */
exports.initSRPAuth = functions.https.onCall((0, error_handler_1.withErrorHandling)(async (data, context) => {
    const { email } = data;
    ...
    const challengeResult = await crypto_1.SRPServer.createAuthChallenge(email, userData.verifier, userData.salt);
    if (!challengeResult.success || !challengeResult.data) {
        throw new functions.https.HttpsError("internal", "Failed to create authentication challenge");
    }
 ...
 */
exports.verifySRPAuth = functions.https.onCall((0, error_handler_1.withErrorHandling)(async (data, context) => {
    ...
    const verificationResult = await crypto_1.SRPServer.verifyClientProof(email, srpProof);
 ...
 */
exports.registerUser = functions.https.onCall((0, error_handler_1.withErrorHandling)(async (data, context) => {
    ...
    const storeResult = await crypto_1.SRPServer.storeUserCredentials(email, verifier, salt);
    if (!storeResult.success) {
        // Cleanup the user record if credential storage fails
        await admin.auth().deleteUser(userRecord.uid);
        throw new functions.https.HttpsError("internal", "Failed to store user credentials");
    }
 ...
 */
exports.changePassword = functions.https.onCall((0, error_handler_1.withErrorHandling)(async (data, context) => {
    ...
    const updateResult = await crypto_1.SRPServer.storeUserCredentials(email, newVerifier, newSalt);
    if (!updateResult.success) {
        throw new functions.https.HttpsError("internal", "Failed to update user credentials");
    }
    ...
    await admin.firestore().collection("users").doc(context.auth.uid).update({
        "security.lastPasswordChange": admin.firestore.FieldValue.serverTimestamp(),
    });
    return {
        success: true,
    };
 ...
 * @return {Promise<admin.auth.UserRecord>} Firebase Auth user record
 */
