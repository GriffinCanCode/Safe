...
exports.adminManageUser = exports.getUserSecurityStatus = exports.updateTwoFactorAuth = exports.updateUserEmail = exports.deleteUserAccount = exports.getUserProfile = exports.updateUserProfile = void 0;
...
exports.updateUserProfile = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "updateProfile", 10);
...
exports.getUserProfile = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "getUserProfile", 20);
        // Get user document
...
exports.deleteUserAccount = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "deleteAccount", 2);
        const { confirmationCode } = data;
        // For security, require confirmation code
        if (!confirmationCode || confirmationCode !== "DELETE_MY_ACCOUNT") {
            throw new functions.https.HttpsError("invalid-argument", "Invalid confirmation code");
        }
...
exports.updateUserEmail = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "updateEmail", 3);
        const { newEmail } = data;
...
exports.updateTwoFactorAuth = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (typeof enabled !== "boolean") {
            throw new functions.https.HttpsError("invalid-argument", "Enabled flag must be boolean");
        }
        ...
        await db
            ...
            "security.twoFactorUpdatedAt": admin.firestore.FieldValue.serverTimestamp(),
 ...
 */
exports.getUserSecurityStatus = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "getSecurityStatus", 10);
        // Get user document
        const userDoc = await db.collection("users").doc(context.auth.uid).get();
        if (!userDoc.exists) {
...
exports.adminManageUser = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "adminManageUser", 10);
        const { targetUserId, action, reason } = data;
        if (!targetUserId || !action) {
            throw new functions.https.HttpsError("invalid-argument", "Target user ID and action required");
        }
        // Get target user
...
function calculateSecurityScore(userData) {
    ...
    if (userData?.security?.lastPasswordChange) {
        const passwordAge = Date.now() - userData.security.lastPasswordChange.toDate().getTime();
...
function generateSecurityRecommendations(userData) {
    ...
    if (!userData?.settings?.twoFactorEnabled) {
        recommendations.push("Enable two-factor authentication for better account protection");
    }
    if (userData?.security?.lastPasswordChange) {
        const passwordAge = Date.now() - userData.security.lastPasswordChange.toDate().getTime();
    ...
    if (userData?.securityAlerts && userData.securityAlerts > 0) {
        recommendations.push("Review and address any security alerts in your account");
    }
