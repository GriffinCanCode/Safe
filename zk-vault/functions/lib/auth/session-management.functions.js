...
exports.cleanupExpiredSessions = exports.validateSession = exports.getActiveSessions = exports.terminateSession = exports.updateSessionActivity = exports.createSession = void 0;
...
exports.createSession = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "createSession", 10);
        const { deviceInfo, geoData } = data;
        // Validate required fields
        if (!deviceInfo || !deviceInfo.userAgent) {
            throw new functions.https.HttpsError("invalid-argument", "Device information required");
        }
 ...
 */
exports.updateSessionActivity = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "updateSession", 60); // Allow frequent updates
        ...
        if (sessionData.expiresAt.toDate() < new Date()) {
            throw new functions.https.HttpsError("deadline-exceeded", "Session has expired");
        }
...
exports.terminateSession = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "terminateSession", 20);
...
exports.getActiveSessions = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Apply rate limiting
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "getActiveSessions", 10);
...
exports.cleanupExpiredSessions = functions.pubsub
    ...
    try {
        ...
        console.log(`Found ${expiredSessionsSnapshot.size} expired sessions to clean up`);
        ...
        await batch.commit();
        console.log(`Successfully cleaned up ${expiredSessionsSnapshot.size} expired sessions`);
        return null;
    }
    catch (error) {
