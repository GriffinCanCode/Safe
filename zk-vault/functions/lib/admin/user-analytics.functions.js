...
exports.getUserSegmentation = exports.getUserActivityTrends = exports.generateUserAnalytics = void 0;
...
exports.generateUserAnalytics = functions.https.onCall(async (data, context) => {
    try {
        ...
        const [totalUsersResult, activeUsersResult, newUsersResult, userSessionsResult, userCountriesResult, filesResult, vaultItemsResult,] = await Promise.all([
            getTotalUsers(),
        ...
        const previousPeriodUsers = await getNewUsers(admin.firestore.Timestamp.fromDate(previousPeriodStart), startTimestamp);
        const userGrowthRate = previousPeriodUsers.count > 0 ?
 ...
 */
exports.getUserActivityTrends = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated and an admin
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        const activityTrends = await Promise.all(buckets.map((bucket) => getActivityForBucket(bucket)));
        return {
            success: true,
...
exports.getUserSegmentation = functions.https.onCall(async (data, context) => {
    try {
        ...
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "admin", 5);
        // Get user segments
        const [powerUsers, regularUsers, lightUsers, inactiveUsers, newUsers] = await Promise.all([
            getPowerUsers(),
            getRegularUsers(),
            getLightUsers(),
            getInactiveUsers(),
        ...
        const totalUsers = powerUsers.count +
            regularUsers.count +
            lightUsers.count +
            inactiveUsers.count;
        const segmentation = {
