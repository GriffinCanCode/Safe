...
exports.optimizeSystem = exports.purgeInactiveUsers = exports.cleanupExpiredFiles = void 0;
...
exports.cleanupExpiredFiles = functions.pubsub
    ...
    try {
        ...
        console.log(`Found ${expiredFilesSnapshot.size} expired files to clean up`);
 ...
 */
exports.purgeInactiveUsers = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Validate input
        if (inactiveDays < 30) {
            throw new functions.https.HttpsError("invalid-argument", "Inactive days must be at least 30");
        }
        ...
        const purgePromises = inactiveUsersSnapshot.docs.map(async (doc) => {
            ...
            const userFilesSnapshot = await db
                ...
                .where("userId", "==", userId)
                .get();
            const fileDeletePromises = userFilesSnapshot.docs.map(async (fileDoc) => {
            ...
            const vaultItemsSnapshot = await db
                ...
                .where("userId", "==", userId)
                .get();
            const vaultDeletePromises = vaultItemsSnapshot.docs.map((doc) => doc.ref.delete());
            ...
            const sharesSnapshot = await db
                ...
                .get();
            const shareDeletePromises = sharesSnapshot.docs.map((doc) => doc.ref.delete());
            await Promise.all(shareDeletePromises);
        ...
        return (0, error_handler_1.handleError)(error, "purgeInactiveUsers");
    }
});
/**
 ...
 */
exports.optimizeSystem = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated and an admin
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        return (0, error_handler_1.handleError)(error, "optimizeSystem");
    }
});
/**
...
async function cleanupOrphanedChunks() {
    try {
        ...
        const orphanedChunks = files.filter((file) => !validChunkPaths.has(file.name));
        console.log(`Found ${orphanedChunks.length} orphaned chunks out of ${files.length} total chunks`);
 ...
 */
async function rebuildDeduplicationIndex() {
    try {
    ...
    try {
        ...
        for (const doc of expiredSessionsSnapshot.docs) {
            ...
            if (batchCount >= 500) {
                try {
                    ...
                    console.error("Error committing batch delete of expired sessions:", error);
                    errors++;
                }
                batchCount = 0;
            }
        ...
        if (batchCount > 0) {
            try {
                ...
                console.error("Error committing final batch delete of expired sessions:", error);
                errors++;
            }
        }
        return { removed, errors };
...
//# sourceMappingURL=maintenance.functions.js.map