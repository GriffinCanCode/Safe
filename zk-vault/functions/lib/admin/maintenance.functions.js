...
exports.optimizeSystem =
  exports.purgeInactiveUsers =
  ...
  try {
    ...
    const purgePromises = inactiveUsersSnapshot.docs.map(async (doc) => {
      ...
      const vaultItemsSnapshot = await db
        ...
        .where("userId", "==", userId)
        .get();
      const vaultDeletePromises = vaultItemsSnapshot.docs.map((doc) =>
        doc.ref.delete(),
      );
      ...
      const sharesSnapshot = await db
        ...
        .get();
      const shareDeletePromises = sharesSnapshot.docs.map((doc) =>
        doc.ref.delete(),
      );
      await Promise.all(shareDeletePromises);
...
exports.optimizeSystem = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (operations.includes("all") || operations.includes("orphaned_chunks")) {
    ...
    if (operations.includes("all") || operations.includes("expired_sessions")) {
