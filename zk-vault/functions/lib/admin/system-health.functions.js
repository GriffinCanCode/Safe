...
exports.resolveSystemAlert = exports.createSystemAlert = exports.runSystemHealthCheck = exports.getSystemHealth = exports.collectSystemMetrics = void 0;
...
exports.collectSystemMetrics = functions.pubsub
    ...
    try {
        ...
        const apiLogsSnapshot = await db
            ...
            .where("timestamp", ">=", oneDayAgo)
            .get();
        const errorRate = apiLogsSnapshot.size > 0 ?
...
exports.getSystemHealth = functions.https.onCall(async (data, context) => {
    try {
        ...
        const historicalMetricsSnapshot = await db
            ...
            .get();
        const historicalMetrics = historicalMetricsSnapshot.docs.map((doc) => doc.data());
        // Calculate trends
        ...
        return (0, error_handler_1.handleError)(error, "getSystemHealth");
    }
});
/**
 ...
 */
exports.runSystemHealthCheck = functions.https.onCall(async (data, context) => {
    try {
        // Ensure user is authenticated and an admin
        if (!context.auth) {
            throw new functions.https.HttpsError("unauthenticated", "Authentication required");
        ...
        // Apply rate limiting for admin operations
        await (0, rate_limiting_1.checkRateLimit)(context.auth.uid, "admin", 2);
        // Run health checks
        ...
        // Process results
        const results = {
            firestore: healthChecks[0].status === "fulfilled" ?
            ...
            auth: healthChecks[2].status === "fulfilled" ?
                ...
                { status: "error" },
            functions: healthChecks[3].status === "fulfilled" ?
        ...
        return (0, error_handler_1.handleError)(error, "runSystemHealthCheck");
    }
});
/**
    ...
    try {
        ...
        if (severity === "critical") {
            ...
            adminsSnapshot.forEach((doc) => {
                ...
                console.log(`Would notify admin ${doc.id} about critical alert: ${message}`);
            });
        }
        return {
            success: true,
        ...
        return (0, error_handler_1.handleError)(error, "createSystemAlert");
    }
});
/**
...
exports.resolveSystemAlert = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Validate input
        if (!alertId) {
            throw new functions.https.HttpsError("invalid-argument", "Missing alertId");
        }
        // Get alert
        ...
        return (0, error_handler_1.handleError)(error, "resolveSystemAlert");
    }
});
/**
...
function calculateTrends(metrics) {
    ...
    const storageGrowth = calculateGrowthRate(oldest.totalStorageUsed, newest.totalStorageUsed);
    const vaultItemGrowth = calculateGrowthRate(oldest.vaultItemCount, newest.vaultItemCount);
    const errorRateTrend = calculateGrowthRate(oldest.errorRate, newest.errorRate);
    return {
        userGrowth,
        storageGrowth,
        vaultItemGrowth,
 ...
 */
function calculateHealthScore(metrics, alertCount) {
    if (!metrics)
    ...
    const userRatio = metrics.userCount > 0 ? metrics.activeUserCount / metrics.userCount : 1;
...
//# sourceMappingURL=system-health.functions.js.map