 ...
 */
export const checkLeakedCredentials = functions.https.onCall(
  async (data, context) => {
    try {
    ...
    try {
      ...
      usersToCheckSnapshot.forEach((userDoc) => {
        ...
        batch.set(taskRef, {
          ...
          emailHash: userData.emailHash, // We store a hash of the email for breach checking
          status: "pending",
 ...
 */
async function simulateBreachCheck(hashPrefix: string): Promise<string[]> {
